...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpGet]
        public IActionResult RestorePassword(string token, string email)
        {
            var model = new RestorePasswordViewModel { Token = token, Email = email };
            ...
            if(ModelState.IsValid)
            {
                if(model.NewPassword.Equals(model.ConfirmPassword))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        if(result.Succeeded)
                        {
                            return View("Success", "Password has been successfully reset.");
                        }
                        else
                        {
                            SetModelStateErrors(result.Errors);
                        ...
                        ModelState.AddModelError("Unknown user", $"There are no users registered with E-mail address '{model.Email}'.");
                }
                else
                    ModelState.AddModelError("Incorrect password", $"New password and confirmation must be same.");
            }
